...
const LoginForm: React.FC<{ isDark: boolean; login: any; googleLogin: any; navigate: any }> = ({ isDark, login, googleLogin, navigate }) => {
    ...
    return (
        <form className="space-y-6 relative z-10" onSubmit={handleSubmit}>
            <div>
                <label className={`block text-xs font-black ${isDark ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-widest mb-2 ml-1`}>
                    Email Address
                </label>
                <Input
                    type="email"
            ...
            <div>
                <div className="flex justify-between items-center mb-2 ml-1">
                    <label className={`block text-xs font-black ${isDark ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-widest`}>
                        Password
                    </label>
                    <Link
                        to="/forgot-password"
                        className={`text-xs font-bold ${isDark ? 'text-emerald-400 hover:text-emerald-300' : 'text-emerald-600 hover:text-emerald-500'} transition-colors`}
                    >
                        Forgot Password?
                    </Link>
                </div>
            ...
            {error && (
                <motion.p
                    ...
                    className="text-sm text-red-400 font-bold text-center"
                >
                    {error}
                </motion.p>
            )}
            ...
            <div className="relative my-8">
                ...
                <div className="relative flex justify-center text-xs uppercase tracking-widest font-bold">
                    <span className={`${isDark ? 'bg-[#1a243a] text-slate-500' : 'bg-white text-slate-400'} px-4 rounded-full`}>
                        Secure Access
                    </span>
                </div>
            </div>
...
export const Login: React.FC = () => {
    ...
    return (
        <div className={`flex min-h-screen flex-col justify-center items-center py-12 px-6 ${isDark ? 'bg-[#0f172a]' : 'bg-[#f8fafc]'} relative overflow-hidden transition-colors duration-500`}>
            ...
            >
                <div className={`backdrop-blur-3xl py-10 px-7 shadow-2xl rounded-[32px] border ${isDark ? 'bg-slate-900/40 border-white/10' : 'bg-white/85 border-slate-200 shadow-slate-200/50'} overflow-hidden group transition-all duration-500`}>
                    ...
                    <div className="relative z-10 text-center mb-10">
                        ...
                        <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} font-medium tracking-tight`}>
                            Access your EnergyLens dashboard
                        </p>
                    </div>

                    <LoginForm isDark={isDark} login={login} googleLogin={googleLogin} navigate={navigate} />

                    <div className={`mt-8 pt-7 border-t ${isDark ? 'border-white/5' : 'border-slate-200'} text-center relative z-10`}>
                        <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} font-bold mb-4 text-sm`}>
                            New to EnergyLens?{' '}
                            <Link to="/signup" className={`${isDark ? 'text-emerald-400 hover:text-emerald-300' : 'text-emerald-600 hover:text-emerald-500'} transition-colors ml-1`}>
                                Create Account
                            </Link>
                        </p>
                    </div>
            ...
            <p className="text-center mt-8 text-xs text-gray-400 dark:text-gray-500 relative z-10">
                &copy; 2024 EnergyLens. All rights reserved.
            </p>
        </div>
    );
};
